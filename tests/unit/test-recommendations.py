...
def _anomaly_df(rows: list[dict]) -> pd.DataFrame:
    return pd.DataFrame(rows, columns=["node_id", "timestamp", "anomaly_score", "severity"])
...
class TestRecommendationToDict:
    def test_has_all_required_keys(self):
        rec = Recommendation(node_id="node_001", type="high_anomaly", severity="high", message="test")
        d = rec.to_dict()
        assert set(d.keys()) == {"node_id", "type", "severity", "message", "generated_at", "metadata"}

    def test_generated_at_is_iso_string(self):
        rec = Recommendation(node_id="n1", type="high_anomaly", severity="high", message="x")
...
class TestAnomalyRecommendations:
    ...
    def test_high_severity_produces_recommendation(self):
        df = _anomaly_df([{"node_id": "node_001", "timestamp": 1000, "anomaly_score": -0.2, "severity": "high"}])
    ...
    def test_medium_severity_produces_recommendation(self):
        df = _anomaly_df([{"node_id": "n2", "timestamp": 2000, "anomaly_score": -0.1, "severity": "medium"}])
        recs = _anomaly_recommendations(df)
        assert len(recs) == 1
        assert recs[0].severity == "medium"

    def test_low_severity_not_included(self):
        df = _anomaly_df([{"node_id": "n3", "timestamp": 3000, "anomaly_score": -0.02, "severity": "low"}])
        recs = _anomaly_recommendations(df)
        assert recs == []

    def test_one_rec_per_node(self):
        df = _anomaly_df([
            {"node_id": "node_001", "timestamp": 1000, "anomaly_score": -0.3, "severity": "high"},
            {"node_id": "node_001", "timestamp": 2000, "anomaly_score": -0.2, "severity": "high"},
        ])
        recs = _anomaly_recommendations(df)
        assert len(recs) == 1
    ...
    def test_worst_score_selected(self):
        df = _anomaly_df([
            {"node_id": "node_001", "timestamp": 1000, "anomaly_score": -0.3, "severity": "high"},
            {"node_id": "node_001", "timestamp": 2000, "anomaly_score": -0.5, "severity": "high"},
        ])
        recs = _anomaly_recommendations(df)
        assert recs[0].metadata["anomaly_score"] == pytest.approx(-0.5)

    def test_message_contains_node_id(self):
        df = _anomaly_df([{"node_id": "node_42", "timestamp": 1000, "anomaly_score": -0.2, "severity": "high"}])
        recs = _anomaly_recommendations(df)
...
class TestHighConsumptionRecommendations:
    ...
    def test_above_threshold_produces_recommendation(self):
        df = _forecast_df([{"node_id": "n1", "timestamp": 1000, "predicted_consumption": 900.0}])
    ...
    def test_below_threshold_no_recommendation(self):
        df = _forecast_df([{"node_id": "n1", "timestamp": 1000, "predicted_consumption": 500.0}])
        recs = _high_consumption_recommendations(df)
        assert recs == []

    def test_message_contains_node_id(self):
        df = _forecast_df([{"node_id": "node_77", "timestamp": 1000, "predicted_consumption": 950.0}])
        recs = _high_consumption_recommendations(df)
...
class TestLoadShiftRecommendations:
    ...
    def test_peak_hours_trigger_recommendation(self):
        rows = [
            {"node_id": "n1", "timestamp": 1714896000000 + i * 3600000, "predicted_consumption": 100.0 + i * 50}
            for i in range(10)
        ]
...
class TestGenerateRecommendations:
    ...
    def test_high_consumption_suppresses_load_shift_for_same_node(self):
        forecast_rows = [
            {"node_id": "n1", "timestamp": 1714896000000 + i * 3600000, "predicted_consumption": 900.0 + i * 10}
            for i in range(10)
        ]
    ...
    def test_combines_anomaly_and_forecast_recs(self):
        anomaly_rows = [{"node_id": "n1", "timestamp": 1000, "anomaly_score": -0.2, "severity": "high"}]
        forecast_rows = [{"node_id": "n2", "timestamp": 1000, "predicted_consumption": 900.0}]
        recs = generate_recommendations(_anomaly_df(anomaly_rows), _forecast_df(forecast_rows))
