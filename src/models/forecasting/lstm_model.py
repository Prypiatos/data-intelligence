...
def create_mock_data(days=30):
    ...
    power = [400 + 200*np.sin(2*np.pi*(h%24)/24) + np.random.normal(0,30) 
             for h in range(days*24)]
    return pd.DataFrame({'timestamp': timestamps, 'power': power,"hour": timestamps.hour, 'day': timestamps.dayofyear})
