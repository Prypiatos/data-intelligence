...
class PredictionRequest(BaseModel):
    ...
    class Config:
        ...
        example = {"power_readings": [400, 420, 450, 480, 500, 470, 420, 380, 350, 340]}
...
def predict(request: PredictionRequest):
    ...
    try:
        ...
        return PredictionResponse(
            forecast=forecast.tolist(), hours_ahead=24, unit="watts"
        )
...
def predict_batch(request: BatchPredictionRequest):
    ...
    try:
        if not request.batch_readings:
            raise HTTPException(
                status_code=400, detail="Empty batch - provide at least one sequence"
            )
        ...
        return BatchPredictionResponse(forecasts=forecasts, count=batch_size)
